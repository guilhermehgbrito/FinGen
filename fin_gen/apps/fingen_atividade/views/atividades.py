...
def lista_atividades(request):
    ...
    return render(
        request,
        "fingen_atividade/atividades/lista.html",
...
def cria_atividade(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            atividade = form.save()
            messages.success(
                request, f'Atividade "{atividade.titulo}" criada com sucesso!'
            )
        ...
        form = AtividadeForm()
        form.fields["carteira"].queryset = Carteira.objects.filter(
            usuario=request.user
        )
        return render(
            request,
            "fingen_atividade/atividades/criar_editar.html",
            {"form": form, "modo": "cria"},
        )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request,
                    f'Atividade "{atividade.titulo}" editada com sucesso!',
                )
                return redirect("lista_atividades")
            else:
                messages.error(request, "Erro ao gravar dados.")
            ...
            return render(
                request,
                "fingen_atividade/atividades/criar_editar.html",
                {"form": form, "modo": "edita", "atividade_id": atividade.id},
            )
