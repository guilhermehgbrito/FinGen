...
def atividades_mean(objects: Dict or None) -> float:
    value = sum(
        [
            y.valor if y.tipo == "P" else y.valor * -1
...
def group_atividades_by_selected_year(
    objects: Iterable[Atividade], year: int = date.today().year
) -> Dict:
    group = {}
    for obj in objects:
        if obj.data_da_atividade.year == year:
            valor = (
                float(obj.valor) if obj.tipo == "P" else float(obj.valor) * -1
            )
            year_key = f"{year}"
            ...
            if group.get(year_key, None) is None:
                ...
                if group[year_key].get(month_key, None) is None:
                    group[year_key].update({month_key: valor})
                else:
                    group[year_key][month_key] += valor
    if len(group) == 1:
        for i in range(1, 13):
        ...
        group[year_key] = {
            x[0]: x[1]
            for x in sorted(group[year_key].items(), key=lambda x: int(x[0]))
        }
    return group


def atitivades_categories_percentage(
    objects: Iterable[Atividade],
    ...
    return [
        (
            k,
            round(v / total_atividades * 100, 2),
            Categoria.objects.get(titulo=k).cor,
        )
