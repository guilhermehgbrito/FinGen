...
from fin_gen.apps.fingen_financeiro.utils import (
    atitivades_categories_percentage,
    atividades_mean,
    group_atividades_by_month,
    group_atividades_by_selected_year,
    group_atividades_by_year,
    is_empty,
)
...
def index(request):
    ...
    atividades_categories = is_empty(
        atividades, atitivades_categories_percentage
    )
    atividades_year_group = is_empty(atividades, group_atividades_by_year)
    atividades_year_month_group = is_empty(
        atividades, group_atividades_by_selected_year
    )
    atividades_line = is_empty(
        atividades_year_month_group, json.dumps, ensure_ascii=False
    )
